...
class _XmlTest(unittest.TestCase):
	...
	def __exe_step(self, driver, oneStep, *args, **awd):
		...
		# step desc
		desc = NoExceptionDict(oneStep, "desc")
		if desc != None:
			print(desc)
		...
		if h5 and h5 == "true":
			...
			if id:
				...
				#ifexist
				ifexist = NoExceptionDict(oneStep, "ifexist")
				if ifexist and ifexist.startswith('@'):
					if( self.__exe_xmlMethod(driver, ifexist[1:]) == False) :
						return False
				#ifnotexist
				ifnotexist = NoExceptionDict(oneStep, "ifnotexist")
				...
				if text:
					...
					if text.startswith('@') :
						try:
							real_text = awd["fields"][text[1:]]
						except:
							pass

					print("get real text%s"%real_text)
